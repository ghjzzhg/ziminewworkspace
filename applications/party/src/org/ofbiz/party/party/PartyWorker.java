...
public class PartyWorker {
    ...
    public static String getPartyName(Delegator delegator, String partyId) throws GenericEntityException {
        ...
        if("PERSON".equals(partyTypeId)){
            party = EntityQuery.use(delegator).from("Person").where("partyId", partyId).cache().queryOne();
            String fullName = party.getString("fullName");
            return UtilValidate.isEmpty(fullName) ? party.getString("lastName") + party.getString("firstName") : fullName;
        }else{
            party = EntityQuery.use(delegator).from("PartyGroup").where("partyId", partyId).cache().queryOne();
            return party.getString("groupName");
        }
    }
