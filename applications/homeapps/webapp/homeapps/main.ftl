...
<div id="wrapper">
  ...
  <div id="form">
    ...
    <#if userLogin??>
      <h2>${uiLabelMap.CommonWelcome} <br />${lastName}${firstName}</h2>
      <br />
    ...
    <#else>
      <h2>${uiLabelMap.CommonBeLogged}</h2>
      ...
      <form id="login" name="login" method="post" action="<@ofbizUrl>login${previousParams?if_exists}</@ofbizUrl>">
          <input type="hidden" name="JavaScriptEnabled" value="N"/>
        <p class="top">
          <label for="username">${uiLabelMap.CommonUsername}</label>
          <input class="inputLogin" type="text" id="username" value="${username}" name="USERNAME" size="50"/>
        </p>
        <p>
          <label for="password">${uiLabelMap.CommonPassword}</label>
          <input class="inputLogin" type="password" id="password"value="${password}" name="PASSWORD" size="50"/>
        </p>
    ...
    <br/><br/>
    <#include "component://homeapps/webapp/homeapps/latestnews.ftl"/>
  </div>
        ...
        <#assign appIndex = appIndex + 1 />
          <div id="button" class="${app.applicationId}" onmouseover="javascript:writeAppDetails('${app.shortName!app.applicationId}','${app.applicationName!app.applicationId}','${app.description!app.applicationId}')">
            <#if app.imageUrl?has_content>
                <a href="${app.linkUrl}<#if app.linkUrl?url('utf-8')?contains("%26%23x3f%3B")>&<#else>?</#if><#if externalKeyParam?exists>${externalKeyParam}</#if>">
                    <img src="${app.imageUrl}" width="150px" height="150px" onmouseover="this.src='${app.imageHoverUrl!app.imageUrl}'" onmouseout="this.src='${app.imageUrl}'" />
                </a>
            </#if>
...
<script language="JavaScript" type="text/javascript">
    document.login.JavaScriptEnabled.value = "Y";
