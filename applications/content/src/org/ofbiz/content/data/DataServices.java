...
public class DataServices {
    ...
    public static List<String> storeAllDataResourceInMap(LocalDispatcher dispatcher, GenericDelegator delegator, GenericValue userLogin, Map<String, Object> fileData){
        return storeDataResource(dispatcher, delegator, userLogin, fileData, false);
    }

    public static List<String> storeDataResource(LocalDispatcher dispatcher, GenericDelegator delegator, GenericValue userLogin, Map<String, Object> fileData, Boolean readOnly){
        ...
        for (Map.Entry<String, Object> entry : fileData.entrySet()) {
            ...
            if(fieldName.startsWith("_") || !(entry.getValue() instanceof ByteBuffer)){//如果文件框未选择文件则忽略
                continue;//以存储文件内容的字段为基准
            }
            String fileName = (String) fileData.get("_" + fieldName + "_fileName");
            if(UtilValidate.isEmpty(fileName)){
                continue;//如果传入的map中有其他键值对需要忽略
            }
            Long fileSize = (Long) fileData.get("_" + fieldName + "_size");
            String fileType = (String) fileData.get("_" + fieldName + "_contentType");
            ByteBuffer fileBytes = (ByteBuffer) entry.getValue();
            DataFileDescription fileDescription = new DataFileDescription(fileName, fileSize, fileType, fileBytes.array());
            fileDescription.setReadOnly(readOnly);
            result.add(fileManager.storeFile(fileDescription, userLogin));
        }
        return result;
    }

    public static List<String> storeAllDataResourceInMap(LocalDispatcher dispatcher, GenericDelegator delegator, GenericValue userLogin, Map<String, Object> fileData, Boolean readOnly){
        return storeDataResource(dispatcher, delegator, userLogin, fileData, readOnly);
    }
     ...
     * @return 文件ID
     */
    public static String storeAllDataResourceInMap(Delegator delegator, String fileName, Long fileSize, String fileType, byte[] content, GenericValue userLogin, String fileId, boolean fileFlag){
        String storeType = UtilProperties.getPropertyValue("content.properties", "content.store.type");
        FileTypeManager fileManager = FileManagerFactory.getFileManager(storeType, (GenericDelegator)delegator);
        DataFileDescription fileDescription = new DataFileDescription(fileName, fileSize, fileType, content,fileId , fileFlag);
        return fileManager.storeFile(fileDescription, userLogin);
    }

    /**
     * 将dataresource中的文件转换为邮件发送的附件
     * @param delegator
     * @param dataResourceIds
     * @return
     */
    public static List<Map<String, ? extends Object>> getDataResourceAsEmailBodyParts(GenericDelegator delegator, List<String> dataResourceIds){
        List<Map<String, ? extends Object>> result = new ArrayList<>();
        if(UtilValidate.isNotEmpty(dataResourceIds)){
            try {
                List<GenericValue> files = EntityQuery.use(delegator).from("DataResource").where(EntityCondition.makeCondition("dataResourceId", EntityOperator.IN, dataResourceIds)).queryList();
                for (GenericValue file : files) {
                    String fileType = file.getString("dataResourceTypeId");
                    FileTypeManager fileManager = FileManagerFactory.getFileManager(fileType, delegator);
                    byte[] content = fileManager.getFileContent(file.getString("dataResourceId"));
                    String fileName = file.getString("dataResourceName");
                    String contentType = file.getString("mimeTypeId");
                    result.add(UtilMisc.toMap("content", content, "type", contentType, "filename",
                            fileName));
                }
            } catch (GenericEntityException e) {
                Debug.logError(e, "获取文件错误", module);
            }
        }
        return result;
    }
