...
var ProcessDiagramGenerator = {
	...
	showFlowInfo: function(flow){
		...
		var processSet = $("#" + this.options.processSetId);
		...
		var TPL_FLOW_INFO = '<div>{flow}</div>'
			+ '<div><b>源id</b>: {sourceActivityId}</div>'
			+ '<div><b>目标id</b>: {destinationActivityId}</div>'
			+ '<div><b>默认</b>: {isDefault}</div>'
			+ '<div><b>有条件</b>: {isConditional}</div>'
			+ '<div><b>高亮</b>: {isHighLighted}</div>';
		var tpl = Lang.sub(TPL_FLOW_INFO, values);
		//console.log("info: ", tpl);
		diagramInfo.html(tpl);
		processSet.hide();
	},
	setFlowInfo: function (flow) {
		var diagramInfo = $("#" + this.options.diagramInfoId);
		var processSet = $("#" + this.options.processSetId);
		if (!diagramInfo) return;
		if (!processSet) return;
		diagramInfo.html(flow);
		processSet.hide();
	},
	showActivityInfo: function(activity){
		...
		//	+ '<div><b>流程Key</b>: {processDefinitonKey}</div>';

		//var template = TPL_ACTIVITY_INFO;
		var template = '<div><b>请选择用户任务节点或者分支下的流程线进行流程信息设置</b></div>'
	...
	showTaskForm: function(procDefId, activityId){
		this.hideInfo();
		...
		pb1.set('value', 0);
		$.ajax({
			url: '/workflow/control/CreateActTaskExtForm',
			type: 'POST',
			dataType: 'html',
			data:{procDefId: procDefId, taskId: activityId},
			cache: false,
			async: true
		}).done(function(data) {
			processSet.html(data);
			pb1.set('value', 100);
		}).fail(function(jqXHR, textStatus){
			//console.error('Get diagram layout['+processDefinitionKey+'] failure: ', textStatus, jqXHR);
		});
	...
	setTaskForm: function(){
		var processSet = $("#" + this.options.processSetId), taskForm = $("#taskForm");
		if (!processSet) return;
		if (!taskForm) return;

		var update = taskForm.attr("formId"),formId = taskForm.val();
		if(!formId){
			alert("请选择表单后保存设置");
			return;
		}
		pb1.set('value', 0);
		$.ajax({
			url: update ? '/workflow/control/updateActTaskExtForm' : '/workflow/control/createActTaskExtForm',
			type: 'POST',
			dataType: 'json',
			data:{procDefId: taskForm.attr("procDefId"), taskId: taskForm.attr("taskId"), formId: formId},
			cache: false,
			async: true
		}).done(function(data) {
			pb1.set('value', 100);
			taskForm.attr("formId", taskForm.val());
		}).fail(function(jqXHR, textStatus){
			//console.error('Get diagram layout['+processDefinitionKey+'] failure: ', textStatus, jqXHR);
		});
	},
	saveTaskUser: function(){
		var processSet = $("#" + this.options.processSetId), taskForm = $("#taskForm");
		if (!processSet) return;
		if (!taskForm) return;

		if($.hrMacro.isEmptyScope({name: 'taskUser'})){
			alert("请选择用户后保存设置");
			return;
		}
		pb1.set('value', 0);
		var param = $.hrMacro.getDataScope({name: 'taskUser'});
		$.ajax({
			url: '/workflow/control/saveTaskUser',
			type: 'POST',
			dataType: 'json',
			data:param,
			cache: false,
	...
	setTaskAssignee: function(){
		...
		displayInTab3("taskAssigneeTab", "设置任务责任人",{requestUrl: "/workflow/control/SetTaskUser", data:{procDefId: taskForm.attr("procDefId"), taskId: taskForm.attr("taskId")}, width: "900px"});
	},

	cancelTaskForm: function(){
		var processSet = $("#" + this.options.processSetId), taskForm = $("#taskForm");
		if (!processSet) return;
		if (!taskForm) return;
		pb1.set('value', 0);
		$.ajax({
			url: '/workflow/control/deleteActTaskExtForm',
			type: 'POST',
			dataType: 'json',
			data:{procDefId: taskForm.attr("procDefId"), taskId: taskForm.attr("taskId")},
			cache: false,
			async: true
		}).done(function(data) {
			pb1.set('value', 100);
			taskForm.attr("formId", "");
			taskForm[0].selectedIndex = 0;
		}).fail(function(jqXHR, textStatus){
			//console.error('Get diagram layout['+processDefinitionKey+'] failure: ', textStatus, jqXHR);
		});
	},
