...
/*组合checkbox*/
function fnParseOptions(id,gChecked)
{
    var oTable = document.getElementById('options_table');
    ...
    for(var i=0;i<trLength;i++)
    {
        ...
        if(inputs.length>0)
        {
            ...
            html += '<input name="'+id+'" value="'+inputs[1].value+'" '+sChecked+' type="checkbox"/>'+inputs[1].value+'&nbsp;';
...
dialog.onok = function (){
    ...
    if( !oNode ) {
        try {
            ...
            var options = fnParseOptions(id,gChecked,gTimestamp);
            if(!options)
            {
                alert('请添加选项');
                return false;
