...
result = runService("getLowerOccupationMembers", positionMap);
members = result.data;
/*recordList = [];
if(members){
    for (Map member  : members) {
        recordList.add([emplyeeName: member.get("employeeName"), post: member.get("post")]);
    }
}*/
context.recordList = members;