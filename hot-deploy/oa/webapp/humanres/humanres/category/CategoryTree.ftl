...
<@treeInline id="category" url="GetOrganizationTree" onselect="onSelectOrg" edit=true addFun="addSubOrg" onUpdateFun="updateOrgName"/>
<script type="text/javascript">


    function afterSaveEmployee() {
        var orgId = "";
        var orgTree = $.treeInline['category'];
        if(orgTree){
            var categoryTree = orgTree.tree;
            var nodes = categoryTree.getSelectedNodes();
            if(nodes && nodes.length){
                orgId = nodes[0].code;
                ajaxUpdateAreas('PersonList,/hr/control/ListDepartmentMembers,direct=true&partyId=' + orgId + '&_=1467606714756&VIEW_SIZE_1=2&VIEW_INDEX_1=1')
            }
        }
    }
    ...
    $(function(){
        ...
        var node = categoryTree.getNodes()[0];
        categoryTree.selectNode(node);
        onSelectOrg(node.id);
    })

    function addSubOrg(data){
        $.organization.addSubOrg(data.parentCode);
    }

    function updateOrgName(data){
        $.organization.updateOrgName(data.id, data.name);
    }
    ...
    function onSelectOrg(id){
        ...
        $.ajax({
            type: 'GET',
            url: "EditOrganization?partyId=" + id,
            async: true,
            dataType: 'html',
        ...
        //刷新岗位
//        $.organization.refreshOccupationDetail(id);
