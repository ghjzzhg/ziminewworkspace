...
<script type="text/javascript">
    ...
    //已预约
    function changeDivBackgroundColor(id, start, end) {
        var j = parseInt(end);
  ...
  $(function () {
    <#list occupyTime as timeList>
        <#if timeList?has_content>
            changeDivBackgroundColor('resource_${timeList.resourceId}_', '${timeList.startDate}', '${timeList.endDate}');
        </#if>
    </#list>
    <#list occupyTimeForThrough as timeListForThrough>
        <#if timeListForThrough?has_content>
            changeDivBackgroundColorForThrough('resource_${timeListForThrough.resourceId}_', '${timeListForThrough.startDate}', '${timeListForThrough.endDate}');
        </#if>
    </#list>
    <#list occupyTimeForArrange as timeListForArrange>
        <#if timeListForArrange?has_content>
            changeDivBackgroundColorForArrange('resource_${timeListForArrange.resourceId}_', '${timeListForArrange.startDate}', '${timeListForArrange.endDate}');
        </#if>
    </#list>
    })
</script>
...
<form name="resourceList">
    <table border="5" width="100%" style="background-color: #FFFFFF;border-color: #FF0000 ">
        <tr style="height: 45px">
            <td style=" font-size: 15px;width: 100px;white-space: nowrap;font-weight:bold">资源名称</td>
    ...
    <#list resourceListShow as list>
                        ...
                        <td>
                            <div>
                                <#list 0..48 as divList>
                                    <div id="resource_${list.resourceId}_${divList}"
                        ...
                        <tr>
                            <td>
                                <div style="width:800px;">
                                    <#list 0..48 as divList>
                                        <div id="resource_time_${list.resourceId}_${divList}"
        ...
        <#list 16..23 as changeDivList>
            <script>
                changeDivBackgroundColorForTime('resource_time_${list.resourceId}_${changeDivList}');
            </script>
        </#list>
