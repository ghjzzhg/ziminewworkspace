...
if (UtilValidate.isNotEmpty(name)) {
    ...
    conditions = [];
    if (UtilValidate.isNotEmpty(type)) {
        conditions.add(EntityCondition.makeCondition("partnerType", EntityOperator.EQUALS, type));
    } else {
        List<String> providerRoles = ["CASE_ROLE_ACCOUNTING", "CASE_ROLE_INVESTOR", "CASE_ROLE_STOCK", "CASE_ROLE_LAW"];
        conditions.add(EntityCondition.makeCondition("partnerType", EntityOperator.IN, providerRoles));
    }
    conditions.add(EntityCondition.makeCondition("groupName", EntityOperator.LIKE, "%" + name + "%"));
