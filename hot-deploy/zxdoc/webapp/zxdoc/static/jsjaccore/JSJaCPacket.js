...
JSJaCPacket.prototype._setChildNode = function (nodeName, nodeValue) {
  ...
  //presence的x结点
  if (nodeName == 'x') {
    //var bNode = this.getDoc().createElement('history');
    //aNode.appendChild(bNode);
    aNode.setAttribute('xmlns', 'http://jabber.org/protocol/muc');
  }
  //message的x结点
  if (nodeName == 'messageXNode') {
    var bNode = this.getDoc().createElement('composing');
    aNode.appendChild(bNode);
    aNode.setAttribute('xmlns', 'jabber:x:event');
  }
  return aNode;
};
...
//JSJaCPacket.prototype.appendChildNode = function(nodeName, element) {
//  if (typeof element=='object') { // seems to be a prebuilt node
//    this.getNode().appendChild(element);
//  } else { // build node
//    this.getNode().appendChild(this.buildNode(element,
 ...
 *设置子节点x
 */
JSJaCPresence.prototype.setXNode = function (xNode) {
  this._setChildNode("x", xNode);
  return this;
}
/**
 ...
 * 设置query的子节点item
 * */
JSJaCIQ.prototype.setQueryItem = function (query, JID, nickName) {
  var item = this.getDoc().createElement('item');
...
JSJaCIQ.prototype.setZdlist = function(JID, keyWords, start, end, limit) {
  var zdlist;
  var xmlns = 'urn:xmpp:archive:auto';
  try {//需使用此方法创建element
    zdlist = this.getDoc().createElementNS(xmlns, 'zdlist');
  } catch (e) {
    zdlist = this.getDoc().createElement('zdlist');
    zdlist.setAttribute('xmlns', xmlns);
  }
  //TODO 参数最好改为用set传递
  zdlist.setAttribute('with', JID);
  zdlist.setAttribute('start', start);
  zdlist.setAttribute('end', end);
  zdlist.setAttribute('limit', limit);
  zdlist.setAttribute('keyWords', keyWords);
  // var set = this.getDoc().createElement('set');
  // set.setAttribute('xmlns', 'http://jabber.org/protocol/rsm');
  // zdlist.appendChild(set);
  this.getNode().appendChild(zdlist);
};
JSJaCIQ.prototype.setKclist = function(conversationId, limit, roomId) {
  var kclist;
  var xmlns = 'urn:xmpp:archive:auto';
  try{
    kclist = this.getDoc().createElementNS(xmlns, 'kclist');
  } catch (e) {
    kclist = this.getDoc().createElement('kclist');
    kclist.setAttribute('xmlns', xmlns);
  }
  kclist.setAttribute('cvid', conversationId);
  kclist.setAttribute('limit', limit);
  kclist.setAttribute('roomid', roomId);
  this.getNode().appendChild(kclist);
};
JSJaCIQ.prototype.setList = function(JID, keyWords) {
  var list = this.getDoc().createElement('list');
  list.setAttribute('xmlns', 'urn:xmpp:archive');
  list.setAttribute('with', JID);
  list.setAttribute('keyWords', keyWords);
  var set = this.getDoc().createElement('set');
  set.setAttribute('xmlns', 'http://jabber.org/protocol/rsm');
  var max = this.getDoc().createElement('max');
  var textNode = this.getDoc().createTextNode('50');
  max.appendChild(textNode);
  set.appendChild(max);
  list.appendChild(set);
  this.getNode().appendChild(list);
};
JSJaCIQ.prototype.setRetrieve = function(JID) {
  var retrieve = this.getDoc().createElement('retrieve');
  retrieve.setAttribute('xmlns', 'urn:xmpp:archive');
  retrieve.setAttribute('with', JID);
  retrieve.setAttribute('start', '2016-09-19T02:51:01.281Z');
  var set = this.getDoc().createElement('set');
  set.setAttribute('xmlns', 'http://jabber.org/protocol/rsm');
  var max = this.getDoc().createElement('max');
  var textNode = this.getDoc().createTextNode('100');
  max.appendChild(textNode);
  set.appendChild(max);
  retrieve.appendChild(set);
  this.getNode().appendChild(retrieve);
}
/**
 * @param query iq对象
 * @param JID 和该JID的所有聊天记录
 */
JSJaCIQ.prototype.setQueryChatRecord = function (query, JID) {
  var set = this.getDoc().createElement('set');
  set.setAttribute('xmlns', 'http://jabber.org/protocol/rsm');
  var max = this.getDoc().createElement('max');
  var maxNum = this.getDoc().createTextNode('50');
  var before = this.getDoc().createElement('before');
  max.appendChild(maxNum);
  set.appendChild(max);
  set.appendChild(before);

  var x = this.getDoc().createElement('x');
  x.setAttribute('xmlns', 'jabber:x:data');
  x.setAttribute('type', 'submit');
  var field1 = this.getDoc().createElement('field');
  field1.setAttribute('var', 'FORM_TYPE');
  field1.setAttribute('type', 'hidden');
  var value1 = this.getDoc().createElement('value');
  var value1val = this.getDoc().createTextNode('urn:xmpp:mam:0');
  var field2 = this.getDoc().createElement('field');
  field2.setAttribute('var', 'with');
  var value2 = this.getDoc().createElement('value');
  var value2val = this.getDoc().createTextNode(JID);
  value1.appendChild(value1val);
  value2.appendChild(value2val);
  field1.appendChild(value1);
  field2.appendChild(value2);
  x.appendChild(field1);
  x.appendChild(field2);
 ...
 *设置子节点x
 */
JSJaCMessage.prototype.setXNode = function (xNode) {
  this._setChildNode("messageXNode", xNode);
  return this;
}
/**
 ...
 *得到reason结点
 */
JSJaCMessage.prototype.getReason = function() {
  return this.getChildVal('reason');
}
/**
