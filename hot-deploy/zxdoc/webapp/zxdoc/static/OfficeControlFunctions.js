...
function openNtko(param){
	var rptParam = typeof(param) == "string" ? {fileId : param, staticFile: true} : param;

	var isIe = detectIE() ? true : false, download = param.download;
	if(!isIe && rptParam.type != "pdf" && !download){
		layer.alert('非IE浏览器无法直接预览', {
			time: 0 //不自动关闭
			...
			,btn1: function(index){
				...
				$.extend(rptParam, {type: "pdf"});

				$.ajax({
					type:'POST',
					url:appCtx +"/ntko/ajax/form?isIE=" + isIe,
					async:true,
					data: rptParam,
					dataType:'html',
					success:function (content) {
						displayInTab("ntkoTab", "在线文档", content, {height:"max"});
					}
				})
			},btn2:function(index){
				layer.close(index);
				$.ajax({
					type:'POST',
					url:appCtx + "/report/ajax/generate/" + rptParam.type,
					data: rptParam,
					async:true,
					dataType:'json',
					success:function (data) {
						downloadServerAttachment(data.fileName);
					}
				})
			}
		});
...
function intializePage(fileUrl, readonly)
{
	/*var domain = window.location.host;
	if(domain.indexOf("localhost") >= 0 || domain.indexOf("127.0.0.1") >= 0 || domain.indexOf("192.168.1.10") >= 0){
		fileUrl = fileUrl;
	}else{
		fileUrl = "http://www.xn--tmz15ibun.com" + fileUrl;
	}*/
	...
	//initCustomMenus();
	NTKO_OCX_OpenDoc(fileUrl, readonly);
}
...
function NTKO_OCX_OpenDoc(fileUrl, readonly)
{
	var r = readonly == undefined ? false : readonly;
	OFFICE_CONTROL_OBJ.BeginOpenFromURL(fileUrl, true, r);
}
...
function saveFileToUrl(myUrl, fileId)
{
	//var fileName = document.all("fileName").value;
	var fileName = fileId + '-' + new Date().getTime();
	var result  ;
	if(IsFileOpened)
	{
