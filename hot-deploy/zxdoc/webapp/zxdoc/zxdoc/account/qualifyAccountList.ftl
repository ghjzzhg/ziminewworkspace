...
<script type="text/javascript">
    ...
    function batchPassQualification() {
        var checkedUser = [];
        ...
        if (checkedUser.length) {
            $.account.passQualification(checkedUser.join(","));
        } else {
            showError("请选择账户后操作！");
        }
    }
    function batchDeleteQualification() {
        var checkedUser = [];
        $("input[name=userLoginId]:checked").each(function () {
            checkedUser.push($(this).val());
        });
        ...
        if (checkedUser.length) {
            $.account.deleteQualification(checkedUser.join(","));
        } else {
            showError("请选择账户后操作！");
        }
    }
...
<script id="operatorCol" type="text/html">
    <a class="btn btn-md grey-salsa btn-outline" href="javascript:$.account.deleteQualification('{{userLoginId}}');" title="拒绝"> <i class="fa fa-ban"></i> </a>
    <a class="btn btn-md green btn-outline" href="#nowhere" onclick="$.account.passQualification('{{userLoginId}}')"
       title="通过"> <i class="fa fa-gavel"></i> </a>
</script>
<script type="text/html" id="url">
    <a href="javascript:displayInLayer('认证信息', {requestUrl: 'ViewQualification?partyId={{partyId}}', height: 600});">{{groupName}}</a>
</script>
<style type="text/css">
    .btn .badge, .btn .label {
        top: -19px;
...
<div class="portlet light ">
    ...
    <div class="portlet-body">
        <div style="margin-bottom: 10px">
            <div class="type-buttons btn-group" data-toggle="buttons">
                <label class="btn blue active">
                    <input type="radio" class="toggle" name="type" value=""> 全部 <span
                        class="badge badge-default hide">0</span></label>
            ...
            <#list enumerations as category>
                <label class="btn blue">
                    <input type="radio" class="toggle" name="type"
                           value="${category.roleTypeId}"> ${category.description}<span class="badge badge-danger hide">0</span>
                </label>
            </#list>
            </div>
        </div>
        ...
        <div class="margin-top-10">
            <a class="btn btn-md grey-salsa" href="javascript:batchDeleteQualification();" title="批量拒绝"> <i class="fa fa-ban"></i> </a>
            <a class="btn btn-md green" href="#nowhere" onclick="batchPassQualification()" title="批量通过"> <i
