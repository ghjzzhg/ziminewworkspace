...
<script type="text/javascript">
    ...
    $(function () {
        accountTable = initDatatables("#accounts", {
            ...
            "serverSide": true,
            "ajax": {
                url: "PartyGroupChooserJson",
                type: "POST",
                data: function (d) {
                    d.name = $("#name").val();
                    d.type = '${parameters.type?default('')}';
                    d.partner = $("#partner").is(":checked");
                }
            },
            "order": [
                [1, 'asc'],
            ...
            "columns": [
                {
                    ...
                    "render": function (data, type, row, meta) {
                        return meta.row + 1 + "<input type='hidden' name='partyId' value='" + row.partyId + "'>" + "<input type='hidden' name='groupName' value='" + row.groupName + "'>";
                    }
                },
                {
    ...
    function confirmSelect(){
        if(selectedId){
            closeCurrentTab({"id" : selectedId, "name": selectedName});
        }else{
            showError("请选择一条记录");
        }
    }
...
<div class="portlet light ">
    <div class="portlet-body">
        ...
        <div class="form-group col-sm-2 col-xs-2">
            <label class="control-label" for="partner">含合伙人</label>
            <div class="input-group">
                <input type="checkbox" name="partner" id="partner" maxlength="20" <#if parameters.defaultCheckPartner?has_content>checked="checked"</#if>/>
            </div>
        </div>
