...
int red = 180 + random.nextInt(70);
int green = 180 + random.nextInt(70);
int blue = 180 + random.nextInt(70);
...
for (int i = 0; i < 4; i++) {
    ...
    //随机获得当前验证码的字符
    char codeChar = codeChars.charAt(random.nextInt(codeLength));
    validationCode.append(codeChar);
    //随机设置当前验证码字符的颜色
    red = 10 + random.nextInt(90);
    green = 10 + random.nextInt(90);
    blue = 10 + random.nextInt(90);
    g.setColor(new Color(red, green, blue));
...
return "success";*/
 ...
 final int WIDTH = 120;//生成的图片的宽度
 final int HEIGHT = 30;//生成的图片的高度
String createTypeFlag = context.get("createTypeFlag");//接收客户端传递的createTypeFlag标识
//1.在内存中创建一张图片
