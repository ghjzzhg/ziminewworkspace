...
function ajaxUpdateAreas(areaCsvString) {
    ...
    for (var i = 0; i < numAreas * 3; i = i + 3) {
        ...
        targetParams = targetParams.replace('?','');
        //TODO:ajax分页时修正VIEW_INDEX 及VIEW_SIZE的paginator问题. 可能会出现某些情况下无法适用的情况，但是目前没有更好的解决办法。
        targetParams = targetParams.split("&");
        var modifiedTargetParams = [];
        for(var i in targetParams){
            var param = targetParams[i];
            if(param.indexOf("VIEW_INDEX_") == 0){
                modifiedTargetParams.push("VIEW_INDEX_1=" + param.split("=")[1])
            }else if(param.indexOf("VIEW_SIZE_") == 0){
                modifiedTargetParams.push("VIEW_SIZE_1=" + param.split("=")[1])
            }else{
                modifiedTargetParams.push(param);
            }
        }
        ajaxUpdateArea(areaId, target, modifiedTargetParams.join("&"));
    }
}
...
function ajaxSubmitFormUpdateAreas(form, areaCsvString) {
    /*var s =   $("#"+form).validationEngine("validate");
    return s;*/
    waitSpinnerShow();
