...
*/
    ...
    /*
     * Tests findByCondition and tests searching on a union view-entity
     */
    public void testUnionViewEntity() throws Exception {
        List<GenericValue> results  = delegator.findList("TestUnionView", null, null, null, null, false);

        for (GenericValue v: results) {
            Map<String, Object> fields = v.getAllFields();
            Debug.logInfo("--------------------------", module);
            //      For values of a map
            for (Map.Entry<String, Object> entry: fields.entrySet()) {
        ...
        // This union is combining the Testing entities with itself, so the union should have twice as many records as the entity itself 
        long testingcount = delegator.findCountByCondition("Testing", null, null, null);
        assertEquals("Number of views should equal number of created entities * 2 in the test.", testingcount * 2, results.size());
    }
}
