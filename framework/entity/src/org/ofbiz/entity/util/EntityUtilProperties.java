...
public class EntityUtilProperties implements Serializable {
    ...
    public static void setSystemPropertyValue(String resource, String name, String value, Delegator delegator) {
        if (resource == null || resource.length() <= 0) {
            return;
        }
        ...
        try {
            GenericValue systemProperty = EntityQuery.use(delegator)
                                                     ...
                                                     .queryOne();
            if (systemProperty != null) {
                systemProperty.set("systemPropertyValue", value);
                systemProperty.store();
            }else{
                delegator.create("SystemProperty", UtilMisc.toMap("systemResourceId", resource, "systemPropertyId", name, "systemPropertyValue", value)).store();
            }
        } catch (Exception e) {
            Debug.logWarning("Could not set a system property for " + name + " : " + e.getMessage(), module);
        }
    }
