...
public final class MacroFormRenderer implements FormStringRenderer {
    ...
    public void renderDisplayField(Appendable writer, Map<String, Object> context, DisplayField displayField) throws IOException {
        ...
        if (UtilValidate.isNotEmpty(description) && size > 0 && description.length() > size) {
            title = description;
            //changed by pzq@20160705: size小于8时会报错
            //TODO:中文此时是编码过的，需要还原后再比较长度
//            description = description.substring(0, size - 8) + "..." + description.substring(description.length() - 5);
    ...
    public void renderDropDownField(Appendable writer, Map<String, Object> context, DropDownField dropDownField) throws IOException {
        ...
        String allowEmpty = "";
        String importability = "";
        String aloneFunction = "";
        String url = "";
        String enumTypeId = "";
        String callback = "";
        ...
        if (dropDownField.getImportability()) {
            importability = "Y";
        }
        if(UtilValidate.isNotEmpty(dropDownField.getAloneFunction())){
            aloneFunction = dropDownField.getAloneFunction();
        }
        if(UtilValidate.isNotEmpty(dropDownField.getUrl())){
            url = dropDownField.getUrl();
        }
        if(UtilValidate.isNotEmpty(dropDownField.getEnumTypeId())){
            enumTypeId = dropDownField.getEnumTypeId();
        }
        if(UtilValidate.isNotEmpty(dropDownField.getCallback())){
            callback = dropDownField.getCallback();
        }
        ...
        sr.append(allowEmpty);
        sr.append("\" importability=\"");
        sr.append(importability);
        sr.append("\" aloneFunction=\"");
        sr.append(aloneFunction);
        sr.append("\" url=\"");
        sr.append(url);
        sr.append("\" enumTypeId=\"");
        sr.append(enumTypeId);
        sr.append("\" callback=\"");
        sr.append(callback);
        sr.append("\" options=");
...
//                    sr.append(titleText);
    ...
    public void renderLookupField(Appendable writer, Map<String, Object> context, LookupField lookupField) throws IOException {
        ...
        if (UtilValidate.isEmpty(lookupPosition) || "topleft".equalsIgnoreCase(lookupPosition)) {
            lookupPosition = "";
        }
    ...
    public void renderNextPrev(Appendable writer, Map<String, Object> context, ModelForm modelForm) throws IOException {
        ...
        if (listSize > 0 && this.javaScriptEnabled) {
            if (ajaxEnabled) {
                ajaxSelectUrl = createAjaxParamsFromUpdateAreas(updateAreas, prepLinkText + "' + (this.value - 1) + '", context);
            } else {
