...
public class MacroMenuRenderer implements MenuStringRenderer {
    ...
    private boolean nestedMenuSelected(List<ModelMenuItem> menuItems, Map<String, Object> context){
        if(UtilValidate.isEmpty(menuItems)){
            return false;
        }else{
            for (ModelMenuItem menuItem : menuItems) {
                if (menuItem.isSelected(context)) {
                    return true;
                }else if(!menuItem.getMenuItemList().isEmpty()){
                    if(nestedMenuSelected(menuItem.getMenuItemList(), context)){
                        return true;
                    }
                }
            }
    ...
    public void renderMenuItem(Appendable writer, Map<String, Object> context, ModelMenuItem menuItem) throws IOException {
        ...
        boolean nestedMenuSelected = false;
        if(containsNestedMenus){
            nestedMenuSelected = nestedMenuSelected(menuItem.getMenuItemList(), context);
        }
        parameters.put("nestedMenuSelected", nestedMenuSelected);
        parameters.put("menuLevel", menuItem.getLevel());
        try {
