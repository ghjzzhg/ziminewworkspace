...
<!-- ${boundaryComment} -->
  ...
  <ul>
    <li>
      <ul id="${id?default('')}_mainMenu" class="menu-parent">
</#macro>
...
<a title="${text}" <#if id?has_content> id="${id}"</#if><#if style?has_content> class="${style}"</#if><#if name?has_content> name="${name}"</#if><#if targetWindow?has_content> target="${targetWindow}"</#if> href="<#if "hidden-form"==linkType>javascript:document.${uniqueItemName}.submit()<#else>${linkUrl}</#if>"><#rt/>
</#if>
<#if imgStr?has_content>${imgStr}</#if><#if text?has_content><span class="text">${text}</span><span class="menu-link-marker"></span></#if><#rt/>
<#if (linkType?has_content && "hidden-form" == linkType) || linkUrl?has_content></a><#rt/></#if>
</#macro>

<#macro renderMenuItemBegin style toolTip linkStr containsNestedMenus nestedMenuSelected=false>
        <li class="<#if style?has_content>${style}</#if><#if containsNestedMenus> submenu</#if> <#if nestedMenuSelected> selected</#if>"<#if toolTip?has_content> title="${toolTip}"</#if>><#if linkStr?has_content>${linkStr}</#if><#if containsNestedMenus><ul></#if><#rt/>
</#macro>
