...
<#macro renderDateTimeField name className alert title value size maxlength id dateType shortDateInput timeDropdownParamName defaultDateTimeString localizedIconTitle timeDropdown timeHourName classString hour1 hour2 timeMinutesName minutes isTwelveHour ampmName amSelected pmSelected compositeType formName mask="" event="" action="" step="" timeValues="" yearpicker=false monthpicker=false>
<span class="view-calendar">
<input type="text" name="${name}" class="Wdate ${className?default("")}" onfocus="WdatePicker(<#if dateType?has_content>{${dateType?default("")}}</#if>)"  <#if event?has_content && action?has_content> ${event}="${action}"</#if> <#--<@renderClass className alert />--><#rt/>
      ...
      <script type="text/javascript">
          Date.prototype.Format = function (fmt) {
            ...
            <#if value?has_content>
                var dateformatValue = "${dateType}";
                if(dateformatValue != ''){
                    var dateFormats = dateformatValue.split(",")[0];
                    var timeFormat${name} =  dateFormats.substring("dateFmt:'".length,dateFormats.length -1);
                    var date = timeFormat${name}.split(" ");
                    if(date.length >=2){
                        timeFormat${name} = date[0] + " " + date[1].toLowerCase()
                    }
                    var s = unescapeHtmlText('${value}');
                    if(s.substring(s.length - 1 , s.length) == ".") {
                        s += "00";
                    }
                    var time1 = new Date(s).Format(timeFormat${name});
                    jQuery("#${id}").val(time1);
                }else{
                    showInfo("请联系管理员添加时间格式");
                }
            </#if>

        //        $(datePicker).css({"position":"inherit","z-index":"99"});
        <#--datePicker${name}.on("dp.change", function(e) {-->
            <#--try{-->
                <#--jQuery("#${id}").val(e.date.format(timeFormat${name}));-->
            <#--}catch (e){-->
                <#--jQuery("#${id}").val("");-->
            <#--}-->
        <#--});-->
...
<#macro renderDropDownField name className alert id importability aloneFunction url enumTypeId callback multiple formName otherFieldName event action size firstInList currentValue explicitDescription allowEmpty options fieldName otherFieldName otherValue otherFieldSize dDFCurrent ajaxEnabled noCurrentSelectedKey ajaxOptions frequency minChars choices autoSelect partialSearch partialChars ignoreCase fullSearch>
    <#if importability =="Y">
    <script language="JavaScript" type="text/javascript">
    jQuery(function() {
        ...
        $('#${id}').parent().find("input").blur(function(){
            $("#${name}hidden").val( $('#${id}').parent().find("input").val());
            if($('#${id}').parent().find("input").val() != ""){
...
<span class="ui-widget" class="jqv">
    ...
    <#--&lt;#&ndash;<option value="${currentValue}">---</option><#rt/>&ndash;&gt;-->
    <#--</#if>-->
      ...
      <#list options as item>
        ...
        <#else>
            <option<#if currentValue?has_content && currentValue == item.key> selected="selected"<#elseif !currentValue?has_content && noCurrentSelectedKey?has_content && noCurrentSelectedKey == item.key> selected="selected"</#if> value="${item.key}">${item.description}</option><#rt/>
        </#if>
      </#list>
        </select>
  </span>
    <#if importability =="Y">
        <input type="hidden" id="${name}hidden" class="validate[required,maxSize[20]]">
  ...
  //--></script>
    ...
    <#if importability =="Y">
        ...
        function saveTypeInfo${name}(){
            var value = $('#${id}').parent().find("input").val();
                ...
                if(!validate){
                    $.ajax({
                        ...
                        success: function (data) {
                            if(data.data.flag = "1"){
                                $("#${id}").append('<option value="'+data.data.enumId+'" selected>'+data.data.description+'</option>')
                                $("#${name}A").css("display","none");
...
<#macro renderCheckField items className alert id allChecked currentValue name event action>
  ...
  <span <@renderClass className alert />><#rt/>
      ...
      ${item.description?default("")}</label>
    </span>
  </#list>
</#macro>
...
<script type="text/javascript">
    $("#${formName}").validationEngine("attach", {promptPosition: "topLeft"});
</script>
...
Parameter: title, String - The dialog title.
...
-->
<#macro renderLookupField name formName fieldFormName className="" alert="false" value="" size="" maxlength="" id="" event="" action="" readonly=false autocomplete="" descriptionFieldName="" targetParameterIter="" imgSrc="" ajaxUrl="" ajaxEnabled=javaScriptEnabled presentation="layer" width="" height="" position="{my: 'center', at: 'center top+15%', of: window}" fadeBackground="true" clearText="" showDescription="" initiallyCollapsed="" lastViewName="main" title="">
...
<span class="field-lookup">
      <input type="hidden" <#if name?has_content> name="${name}value"/></#if>
                ...
                jQuery(document).ready(function(){
                    var options = {
                        ...
                        formName : "${formName?html}",
                            width : "${width}",
                            height : "${height}",
                            position : "${position}" ? "${position}" : {my: 'top', at: 'center top+15%', of: window},
  ...
  <#if ajaxEnabled?has_content && ajaxEnabled && (presentation?has_content && presentation == "window") && ajaxUrl?has_content>
...
<#macro renderNextPrev paginateStyle paginateFirstStyle viewIndex highIndex listSize viewSize ajaxEnabled javaScriptEnabled ajaxFirstUrl firstUrl paginateFirstLabel paginatePreviousStyle ajaxPreviousUrl previousUrl paginatePreviousLabel pageLabel ajaxSelectUrl selectUrl ajaxSelectSizeUrl selectSizeUrl commonDisplaying paginateNextStyle ajaxNextUrl nextUrl paginateNextLabel paginateLastStyle ajaxLastUrl lastUrl paginateLastLabel paginateViewSizeLabel>
  ...
  <div class="${paginateStyle}">&nbsp;
      <ul>
          <li class="${paginateFirstStyle}<#if viewIndex gt 0>" onclick="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxFirstUrl}')<#else>submitPagination(this, '${firstUrl}')</#if>"><a href="javascript:void(0)" onclick="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxFirstUrl}')<#else>submitPagination(this, '${firstUrl}')</#if>"><span>${paginateFirstLabel}</span></a><#else>-disabled"><span>${paginateFirstLabel}</span></#if></li>
          <li class="${paginatePreviousStyle}<#if viewIndex gt 0>" onclick="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxPreviousUrl}')<#else>submitPagination(this, '${previousUrl}')</#if>"><a href="javascript:void(0)" onclick="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxPreviousUrl}')<#else>submitPagination(this, '${previousUrl}')</#if>"><span>${paginatePreviousLabel}</span></a><#else>-disabled"><span>${paginatePreviousLabel}</span></#if></li>
        <#if listSize gt 0 && javaScriptEnabled>
            <li class="nav-page-select">
            <#--<select name="page" size="1" onchange="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxSelectUrl}')<#else>submitPagination(this, '${selectUrl}'+this.value)</#if>"><#rt/>
      <#assign x=(listSize/viewSize)?ceiling>
        <#list 1..x as i>
          <#if i == (viewIndex+1)><option selected="selected" value="<#else><option value="</#if>${i-1}">${i}</option>
        </#list>
      </select>-->
                <input name="page" type="text" value="${viewIndex+1}" onchange="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxSelectUrl}')<#else>submitPagination(this, '${selectUrl}'+(this.value - 1))</#if>">
            </li>
        </#if>
          <li class="${paginateNextStyle}<#if highIndex lt listSize>" onclick="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxNextUrl}')<#else>submitPagination(this, '${nextUrl}')</#if>"><a href="javascript:void(0)" onclick="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxNextUrl}')<#else>submitPagination(this, '${nextUrl}')</#if>"><span>${paginateNextLabel}</span></a><#else>-disabled"><span>${paginateNextLabel}</span></#if></li>
          <li class="${paginateLastStyle}<#if highIndex lt listSize>" onclick="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxLastUrl}')<#else>submitPagination(this, '${lastUrl}')</#if>"><a href="javascript:void(0)" onclick="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxLastUrl}')<#else>submitPagination(this, '${lastUrl}')</#if>"><span>${paginateLastLabel}</span></a><#else>-disabled"><span>${paginateLastLabel}</span></#if></li>
        <#if javaScriptEnabled><li class="nav-pagesize">${paginateViewSizeLabel} <select name="pageSize" size="1" onchange="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxSelectSizeUrl}')<#else>submitPagination(this, '${selectSizeUrl}')</#if>"><#rt/>
          ...
          <li class="nav-displaying">${commonDisplaying}${(listSize/viewSize)?ceiling}页</li>
      </ul>
  </div>
  </#if>
</#macro>
