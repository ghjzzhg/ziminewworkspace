...
public class LoginWorker {
    ...
    public static Map<String, HttpSession> userSessions = new ConcurrentHashMap<String, HttpSession>();
     ...
     * 只在有页面渲染(输出)的时候重新获取一个新的externalLoginKey；同一个session只维护一个key，不同的component的session相互独立
     * 即每个component都有一个key。
     * ajax请求(包括通过iframe发出的请求)沿用原session中的key，不重新生成
     */
    public static String getExternalLoginKey(HttpServletRequest request) {
        ...
        synchronized (session) {
            ...
            if (sesExtKey != null) {//如果是ajax请求，不必要重新生成一个新的externalKey
                //修改：如果是iframe类型的请求，也不触发externalLoginKey失效
                if (isAjax(request) && externalLoginKeys.containsKey(sesExtKey)) return sesExtKey;
    ...
    public static void cleanupExternalLoginKey(HttpSession session) {
        ...
        if (userLogin != null && "true".equals(UtilProperties.getPropertyValue("security", "security.login.user.single"))) {
            HttpSession loginSession = LoginWorker.userSessions.get(userLogin.getString("userLoginId"));
            if(loginSession != null && loginSession.getId().equals(session.getId())){
                LoginWorker.userSessions.remove(userLogin.getString("userLoginId"));
            }
        }
    }
    ...
    public static String login(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (ModelService.RESPOND_SUCCESS.equals(result.get(ModelService.RESPONSE_MESSAGE))) {
            ...
            //只允许一次登陆
            if ("true".equals(UtilProperties.getPropertyValue("security", "security.login.user.single"))) {
                HttpSession previousLoginSession = userSessions.remove(userLogin.getString("userLoginId"));;
                if(previousLoginSession != null){
                        try {
                        previousLoginSession.invalidate();
                        } catch (Throwable e) {
                            Debug.logWarning(e, "会话已过期", module);
                        }
                    }
                userSessions.put(userLogin.getString("userLoginId"), session);
            }
            ...
            try {
                Map<String, Object> vipDueMap = dispatcher.runSync("VIPDue", UtilMisc.toMap("userLogin", userLogin));
                if(UtilValidate.isEmpty(vipDueMap.get("data")) || "0".equals(vipDueMap.get("data"))){
                    request.setAttribute("_ERROR_MESSAGE_", "您的账户已过期，请联系管理员续费！");
                    return "error";
                };
            } catch (Exception e) {
                request.setAttribute("_ERROR_MESSAGE_", "获取用户使用期限失败，请重试！");
                return "error";
            }
            ...
            try {
                visitorCookie = new Cookie("ofbiz.current.username", URLEncoder.encode(username, "UTF-8"));
    ...
    public static String autoLoginSet(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (userLogin != null) {
            ...
            try {
                autoLoginCookie = new Cookie(getAutoLoginCookieName(request),  URLEncoder.encode(userLogin.getString("userLoginId"), "UTF-8"));
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
            ...
            LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
            return autoLoginCheck(delegator, session, userLogin.getString("userLoginId"),dispatcher);
        } else {
            return "success";
        }
    }
    ...
    public static String autoLoginCheck(HttpServletRequest request, HttpServletResponse response) {
        ...
        HttpSession session = request.getSession();
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        return autoLoginCheck(delegator, session, getAutoUserLoginId(request), dispatcher);
    }

    private static String autoLoginCheck(Delegator delegator, HttpSession session, String autoUserLoginId, LocalDispatcher dispatcher) {
        if (autoUserLoginId != null) {
            ...
            try {
                ...
                scoreMessage.setEventName("SCORE_RULE_1");
                if(autoUserLogin != null)  scoreMessage.setScoreTarget(autoUserLogin.getString("partyId"));
                dispatcher.runSync("sendScoreMessage",UtilMisc.toMap("message",scoreMessage,"userLogin",autoUserLogin));*/
            } catch (GenericEntityException e) {
    ...
    public static boolean isAjax(HttpServletRequest request) {
        //ajax请求，或者请求中ajax=true或iframe=true的
       return "true".equalsIgnoreCase(request.getParameter("iframe")) || "true".equalsIgnoreCase(request.getParameter("ajax")) || "XMLHttpRequest".equals(request.getHeader("X-Requested-With"));
    }
     ...
     * @return String
     */
    public static String checkSessionExpire(HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();
        Object userLoginSession = session.getAttribute("userLogin");
        if(userLoginSession == null){
            return "expired";
        }
        //禁用账户处理
        GenericValue user = (GenericValue)userLoginSession;
        String userLoginId = (String) user.get("userLoginId");
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        String enabled = "Y";
        try {
            GenericValue nowState = EntityQuery.use(delegator).from("UserLogin").where("userLoginId", userLoginId).queryOne();
            enabled = (String)nowState.get("enabled");
        }catch (Exception e)
        {
        ...
        request.setAttribute("session-active", true);
        return "success";
    }
     ...
     * @return String
     */
    public static String sessionExpireLogin(HttpServletRequest request, HttpServletResponse response) {
        String login = login(request, response);
        if("success".equals(login)){
            request.setAttribute("loginSuccess", true);
            request.setAttribute("userLogin", request.getSession().getAttribute("userLogin"));
            request.setAttribute("externalLoginKey", getExternalLoginKey(request));
        }else{
            request.setAttribute("loginError", request.getAttribute("_ERROR_MESSAGE_"));
        }
        return "success";
    }
}
