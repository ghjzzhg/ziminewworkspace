    ...
    public static Map<String, Object> sendMail(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        if (sendType == null || sendType.equals("mail.smtp.host")) {
            ...
            if (UtilValidate.isEmpty(authUser)) {
                try {
                    String mailAccount = sendFrom;
                    if(sendFrom.contains("<")){
                        mailAccount = sendFrom.substring(sendFrom.indexOf("<") + 1, sendFrom.indexOf(">"));
                    }
                    GenericValue mailAccountRecord = delegator.findOne("UserMailAccount", UtilMisc.toMap("userAccount", mailAccount), true);
                    if(mailAccountRecord != null){
                        authUser = mailAccount;
                        authPass = mailAccountRecord.getString("pass");
                        String smtpHost = mailAccountRecord.getString("smtpHost");
                        if(UtilValidate.isNotEmpty(smtpHost)){
                            sendVia = smtpHost;
                        }
                        String smtpPort = mailAccountRecord.getString("smtpPort");
                        if(UtilValidate.isNotEmpty(smtpPort)){
                            port = smtpPort;
                        }
                    }
                } catch (GenericEntityException e) {
                    String err = "Error finding mail account for[" + sendFrom + "]";
        ...
        try {
            ...
            InternetAddress sender = new InternetAddress(authUser);
            ...
            if (UtilValidate.isEmpty(sender.getPersonal()) && !sendFrom.contains("<")) {
                sender.setPersonal(sendFrom.contains("<") ? sendFrom.substring(0, sendFrom.indexOf("<")) : sendFrom);
            }
            ...
            mail.setFrom(authUser);
            mail.setSubject(subject, "UTF-8");
            mail.setHeader("X-Mailer", "Rextec OA");
            mail.setSentDate(new Date());
            mail.addRecipients(Message.RecipientType.TO, convertMultipleAddress(sendTo));

            if (UtilValidate.isNotEmpty(sendCc)) {
                mail.addRecipients(Message.RecipientType.CC, convertMultipleAddress(sendCc));
            }
            if (UtilValidate.isNotEmpty(sendBcc)) {
                mail.addRecipients(Message.RecipientType.BCC, convertMultipleAddress(sendBcc));
            }
        ...
        try {
            Transport.send(mail, authUser, authPass);
...
//            trans.close();
            ...
            //加层级限制
            int i = 0;
            while ((nestedException = e.getNextException()) != null && nestedException instanceof MessagingException && i < 10) {
                i++;
    ...
    private static Address[] convertMultipleAddress(String email) throws AddressException {
        String[] sendToArray = email.split(",");
        List<Address> sendTos = new ArrayList<>();
        for (String s : sendToArray) {
            sendTos.add(new InternetAddress(s));
        }
        return sendTos.toArray(new Address[0]);
    }
    ...
    public static Map<String, Object> sendMailFromScreen(DispatchContext dctx, Map<String, ? extends Object> rServiceContext) {
        ...
        List<String> attachDataResource = UtilGenerics.checkList(serviceContext.remove("attachDataResource"));
        List<Map<String, ? extends Object>> bodyParts = UtilGenerics.checkList(serviceContext.remove("bodyParts"));
        ...
        if (UtilValidate.isNotEmpty(xslfoAttachScreenLocationList)) {
            if(bodyParts == null){
                bodyParts = new LinkedList<Map<String, ? extends Object>>();
            }
        ...
        if(UtilValidate.isNotEmpty(bodyParts)){
            ...
            if (bodyText != null) {
                ...
                bodyParts.add(0, UtilMisc.<String, Object>toMap("content", bodyText, "type", "text/html"));
            } else {
                bodyParts.add(0, UtilMisc.<String, Object>toMap("content", bodyWriter.toString(), "type", "text/html"));
            }
        } else {
        ...
        try {
            boolean isMultiPart = UtilValidate.isNotEmpty(bodyParts);
            if (!hideInLog) {
                if (isMultiPart) {
