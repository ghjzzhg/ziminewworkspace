...
<script language="JavaScript">
    ...
    <#if parameters.defaultFolderId?has_content && parameters.folderId?has_content>
    	otherModules = "";
	</#if>
    var fileSharePartyId = "${fileSharePartyId?default('')}";//文件/文件夹实际所属人
    var filePathList = unescapeHtmlText("${filePathList?default('')}");
    var defaultFolderId = "${parameters.defaultFolderId}";
    var folderId = "${parameters.folderId}";
     ...
     */
    $(function () {
        /*if(window.name && window.name.indexOf("layui") > -1){
            //在弹出框中限定文件列表高度
            $("#fileListWrapper").height($(window).height() - 50).css("overflow-y", "auto");
        }*/
        if(otherModules == "1" ){//其它模块进入，不显示文件管理左侧类型入口
            if(accessFolderId){
                if(!isFolderOwner){//如果是所属人及其子账号访问，直接访问私有文档，反之访问共享文档
                    $("#defaultFolder").val("2");
                }else{
                    $("#defaultFolder").val("1");
                }
                showFileList(accessFolderId,"",fileSharePartyId);
            }else{
                if(!isFolderOwner){//如果是所属人及其子账号访问，直接访问私有文档，反之访问共享文档
                    setDefault("2");
                }else{
            ...
            $.ajax({
                type: 'GET',
                url: "/ckfinder/control/action?command=init&externalLoginKey=${externalLoginKey}",
                async: true,
                dataType: 'xml',
                success: function (content) {
                    var dataType = xmlToJson(content);
                    appendFolder(dataType);
                }
            });
        }

        $("#fileListWrapper").on("mouseover", "tr", function(){
            $(this).find(".file-tool").show();
        }).on("mouseout", "tr", function(){
            $(this).find(".file-tool").hide();
        })
    })
    ...
    function initTreeTable(){
        folderTable = $("#folderList").treetable({
            ...
            onNodeExpand: function() {
            	var defaultFolderId = $("#defaultFolder").val();
                var node = this;
                if(!$("tr[data-tt-id=" + node.id + "]").data("ajaxDone")){
                    $.ajax({
                        ...
                        dataType: 'xml',
                        cache:false,
                        data:{command:"getFolders", defaultFolderId: defaultFolderId, folderId : node.id, partyId: "", fileSharePartyId: "", externalLoginKey:'${externalLoginKey?default('')}'},
                        success: function (content) {
...
//        showOrHidDivByFolderType(folderId);
    ...
    function showFileList(id,folderName,partyId){
        ...
        $("#folderId").val(id);
        //得到拼接的文件夹名称和ID
        var folderHid = $("#folderPathHid").val();
        //如果name不等于"/"表示进入文件夹分类中下层文件夹
        if((id != "1" && id != "2" && id != "3") || (otherModules && id != "2" && id != "1")){
            //拼接文件夹名称和ID
            var historyPath = folderHid.split("|").filter(function(el) {return el.length != 0}), newPath = folderName + "~" + id + "?" + partyId, index = historyPath.indexOf(newPath);
            if(index > -1){
                historyPath.splice(index, 1);
            }
            historyPath.push(newPath);
            folderHid = historyPath.join("|") + "|";
        }else{
            //如果name等于"/"表示进入文件夹分类的顶层文件夹，需还原设置
            folderHid = "";
            $("#folderPathHid").val("");
        }
        $("#folderPathHid").val(folderHid);
...
//        showOrHidDivByFolderType(id);
     ...
     *
     */
    function ajaxSearchFiles(folderId, partyId){
        //得到默认的文件夹分类（个人文档）
        var defaultFolderId = $("#defaultFolder").val();
        $.ajax({
            type: 'post',
            ...
            dataType: 'xml',
            cache:false,
            data:{command:"getFiles", defaultFolderId:defaultFolderId, folderId : folderId, partyId: partyId, fileSharePartyId: fileSharePartyId, otherModules:otherModules, moduleType: '<#if parameters.caseId?has_content>case</#if>', moduleId: '${parameters.caseId?default('')}', filePathList:filePathList,externalLoginKey:'${externalLoginKey?default('')}'},
            success: function (content) {
                //解析后台传出的xml文件转化为json数据，创建列表，设置是否需要显示文件路径
                var folderPermission = "";
                if(content != null){
                    var fileList = xmlToJson(content);
                    $("img[id^='photo_']").attr("src","");
                    folderPermission = createTableData(fileList);
                    showFolderPath();
                    var openLayer = getLayer();
                    var layerIndex = openLayer.getFrameIndex(window.name);
                    if(layerIndex != undefined){
                        setTimeout(function(){
                            openLayer.iframeAuto(layerIndex);
                        }, 200);
                    }
                ...
                $("#attachment_createFile_value").attachment("destroy");
                if(folderPermission && setFilePermissions(folderPermission, 4)){//具有上传权限时初始化
                    ...
                    var fileStatus = "";//TODO:文件已存在，覆盖还是新版本
                    var fileId = "";//TODO:已存在文件id

                    $("#createFile").attachment({
                        showLink: false,
                        wrapperClass: 'btn btn-success',
                        url: '/ckfinder/control/action?command=fileUpload&fineupload=true&rootFolderId=' + rootId + '&currentFolder=' + folderId + "&fileStatus=" + fileStatus + "&fileId=" + fileId + "&fileSharePartyId=" + fileSharePartyId + "&otherModules=" + otherModules + "&folderStructure=" + folderIds,

            fineUploader: {
                onTruncateComplete: function(){
                },
                onAllComplete: function(succeeded, failed){
                    ...
                    /*if(checkFileStatus(folderId, rootId, name)){
                        var choice = "";
                        if(confirm("文件。。。")){
                            return true;
                        }else{
                        ...
                        /!*layer.confirm("文件 " + name + " 已存在，请选择操作", {
                            shade: 0.2,
                            btn: ['覆盖','新版本'] //按钮
                        }, function(){
                            return true
...
<#--选择按钮-->
<div id="fileListWrapper" class="col-xs-12">
