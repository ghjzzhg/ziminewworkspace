...
<script language="JavaScript">
    ...
    var defaultFolderId = "${parameters.defaultFolderId?default('')}";
    var folderId = "${parameters.folderId?default('')}";
        ...
        if(otherModules == "1" ){//其它模块进入，不显示文件管理左侧类型入口
            ...
            $.ajax({
                ...
                success: function (content) {
                    ...
                    appendFolder(dataType,defaultFolderId,folderId);
                }
            });
        }
     ...
     * @param data 文件信息集合
     */
    function appendFolder(data,defaultFolderId,folderId){
        var folderHtml = "";
        ...
        if(folderList != null ){
            if(folderList.length){
                for(var i = 0; i < folderList.length; i++){
                    ...
                    if(folderId == "1"){
                        $("#defaultFolder").val(defaultFolderId);
                    }
...
<#--隐藏域-->
    ...
    <input type="hidden" name="defaultFolder" id="defaultFolder" value="${parameters.defaultFolderId?default('')}">
