...
public class ResourceType {
	...
	public ResourceType(final ResourceType type,String person) {
		super();
		this.name = type.name;
		if(null != this.name && ("个人文档".equals(this.name) || "发送文件".equals(this.name) || "接收文件".equals(this.name))){
			this.path = type.path+"/"+person+"/";
			this.url = type.url+person+"/";
		}else{
			this.path = type.path;
			this.url = type.url;
		}
