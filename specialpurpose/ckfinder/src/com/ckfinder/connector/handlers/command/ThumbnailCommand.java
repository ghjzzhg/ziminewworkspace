...
public class ThumbnailCommand extends Command {
	...
	private void createThumb() throws ConnectorException {
		...
		try {
			if (!thumbFile.exists()) {
				String path = "";
				if("他人分享".equals(this.type) || "本人分享".equals(this.type)){
					GenericValue file = EntityQuery.use(delegator).select().from("DataResource").where(EntityCondition.makeCondition("dataResourceId",this.fileId)).distinct().queryOne();
					path = file.get("parentObjectInfo").toString() +  file.get("dataResourceName").toString();
				}else{
					 path = configuration.getTypes().get(this.type).getPath()
							+ this.currentFolder + this.fileName;
				}
				File orginFile = new File(disc + path);
				if (!orginFile.exists()) {
					throw new ConnectorException(
							Constants.Errors.CKFINDER_CONNECTOR_ERROR_FILE_NOT_FOUND);
				}
