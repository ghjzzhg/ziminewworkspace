...
public class FileUploadFilter implements Filter {
	...
	private void setSessionCookie(HttpServletResponse httpResponse,
		...
		if (httpRequest.getParameter(getSessionParameterName()) != null) {
			String cookieValue = httpRequest.getParameter(getSessionParameterName());
			Cookie userCookie = new Cookie(getSessionCookieName(), validateCookieValue(cookieValue) );
			httpResponse.addCookie(userCookie);
		}
	}

	private String validateCookieValue ( final String cookieValue ) {
		return cookieValue.replaceAll("[^._=0-9A-Za-z+\\-]", "");
	}
